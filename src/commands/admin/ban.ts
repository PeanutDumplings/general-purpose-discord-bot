...
const command = async (interaction: ChatInputCommandInteraction) => {
    ...
    if (user?.user.id === interaction.client.user.id) {
        return await interaction.reply({
            embeds: [
                serverEmbed.setDescription(`${config.emojis.error} You cannot ban this bot`).setColor(config.colours.failure as ColorResolvable),
            ],
            ephemeral: true,
        });
    }
    ...
    if (!user?.bannable) {
        return await interaction.reply({
            embeds: [
                serverEmbed
                    .setDescription(`${config.emojis.error} ${user} cannot be banned by me`)
                    .setColor(config.colours.failure as ColorResolvable),
            ],
            ephemeral: true,
        });
    ...
    return await interaction.reply({
        embeds: [
            new EmbedBuilder()
                .setDescription(`${config.emojis.success} ***${user}*** was banned`)
